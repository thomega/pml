...
val all_ok : ('a, 'e) result list -> ('a list, 'e) result
(** Check if all results in the argument list are [Ok] and
    return the first [Error] if not.  *)
...
(** Like [List.iter] but with early exit error handling.
    The returned error will be the one of the {e first}
    element in the list causing an error.
    {e Not} tail recursive and not even tail-mod-cons,
    because we must inspect the results. *)
