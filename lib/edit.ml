...
module Perl_s =
  struct
    ...
    let split_substitution s =
      let myname = "Perl_s.of_string" in
    ...
    let%test _ = split_substitution "/ab" = Error ("Perl_s.of_string: missing second '/'")
    let%test _ = split_substitution "/a/b" = Error ("Perl_s.of_string: missing third '/'")
    ...
    let%test _ = split_substitution "/a/b/c/" = Error ("Perl_s.of_string: too many '/'s")

    let of_string text =
      let open Result.Syntax in
      let* rex, sub, flags = split_substitution text in
      if not (List.mem flags [""; "g"; "i"; "ig"; "gi"]) then
        Error (Printf.sprintf "%s: invalid flags \"%s\" in \"%s\"" "Perl_s.of_string" flags text)
      else
    ...
    let%test _ = exec' "/a/b/x" "Aa" = Error "Perl_s.of_string: invalid flags \"x\""
