...
(** Note that we are not using the tag [PERFORMER], but [ARTIST] instead,
    because vlc ignores [PERFORMER] completely.  Therefore we are also
    ignoring [tags.artist] (usually the composer), because it is usually
    repeated as the first element of [tags.performers].

    TODO test this on more albums. *)
let vorbiscomments opt tags =
  List.append
...
let rip_synchronously ?verbose ?dry ?device ?(extra_args=[]) ?(icedax=false) i wav =
  if icedax then
    let device =
      ...
      | None -> [] in
    let args = extra_args @ device @ ["-O"; "wav"; "-t"; string_of_int i; wav] in
    synchronously ?verbose ?dry "icedax" args
  else
    let device =
      match device with
      | Some device -> ["-d"; device]
      | None -> [] in
...
let encode_track ?dry ?verbose bitrate encoders dir d t =
  ...
  and title =
    ...
    | Multi _ ->
       ...
       | "" ->
          begin match d.Tagged.titles with
          | t :: _ -> Tagged.title_to_string t
          | [] -> "Untitled"
          end
       | title ->
          begin match d.Tagged.titles with
          | Tagged.Tracks prefix :: _ -> prefix ^ ": " ^ title
          | _ -> title
          end
       end
  and performers =
